...
class EmployeeRepository {
	...
	const result = await db.query('INSERT INTO employees (first_name, last_name, email, is_available, role) VALUES ($1, $2, $3, $4, $5) RETURNING *', [firstName, lastName, email, isAvailable, role]);
	...
	const result = await db.query('UPDATE employees SET first_name = $1, last_name = $2, email = $3, is_available = $4, role = $5 WHERE employee_id = $6 RETURNING *', [firstName, lastName, email, isAvailable, role, id]);
	if (result.rows.length === 0 ) return null;
	const { employee_id, first_name, last_name, email: employeeEmail, is_available, role: employeeRole } = result.rows[0];
	return new Employee(employee_id, first_name, last_name, employeeEmail, is_available, employeeRole);
    }
