...
class AssignmentController {
	...
	const assignment = await assignmentService.createAssignment(projectId, employeeId, cost);
	...
	const assignment = await assignmentService.updateAssignment(id, projectId, employeeId, cost, assignedAt, finishedAt);
	if (!assignment) return res.status(404).json({ message: 'Assignment not found'});
